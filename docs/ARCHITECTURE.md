...
10.1. [Kotlin Scripting Engine](#101-kotlin-scripting-engine)
10.2. [Extended Metadata Model (ERP)](#102-extended-metadata-model-erp)
...
graph LR
    subgraph backend["backend/ (Gradle modules)"]
        ...
        KotlinScripting["kotlin-scripting\nCompiler service + CompiledScriptCache\nSandbox (allowlist + classloader + runtime guard)\nExecutionContext / RecordOperations\n(ADR-005)"]
    ...
    Automation --> KotlinScripting
    KotlinScripting --> Persistence
    KotlinScripting --> QueryEngine
...
## 10.1. Kotlin Scripting Engine

Full design and rationale in **ADR-005**. This is the `UserCodeExecutor` implementation for imperative logic (Triggers, Batch jobs, Scheduled jobs, Invocable Actions) — the DSL above remains the engine for Validation Rules and simple field automations.

```mermaid
flowchart TD
    Admin["Admin / Developer\n(or AI proposal, §11)"]
    Editor["Script Editor\n(frontend/script-editor, Monaco)"]
    Save["Save md_script row"]

    subgraph Compile["Compile-on-save (boundedElastic)"]
        Hash["SHA-256(source + scriptDefinitionVersion)"]
        CacheCheck["CompiledScriptCache\n(Caffeine, tenant-scoped)"]
        Host["kotlin-scripting-jvm-host\nScriptCompilationConfiguration\n(import allowlist)"]
        Diagnostics["Compile diagnostics\n→ md_script.compile_errors"]
    end

    Activate["Human activates\nmd_script.is_active = true\n(only if compile is clean)"]

    subgraph Trigger["Trigger / Batch / Scheduled execution"]
        Resolve["AutomationEngine resolves\nactive md_script for (object, event)"]
        CacheHit["CompiledScriptCache.get\n(common path: cache hit)"]
        Sandbox["3-level sandbox\ncompile allowlist + restricted classloader\n+ runtime guard (CPU/heap/timeout/recursion)"]
        Exec["Mono.fromCallable { script.execute(ctx) }\n.timeout(5s default)\n.subscribeOn(Schedulers.boundedElastic())"]
        Log["script_execution_log\n(same transaction as the triggering write)"]
    end

    Admin --> Editor --> Save --> Hash --> CacheCheck
    CacheCheck -->|miss| Host --> Diagnostics --> Activate
    CacheCheck -->|hit, unchanged| Activate
    Activate --> Resolve --> CacheHit --> Sandbox --> Exec --> Log

    style Sandbox fill:#fff3cd,stroke:#ffc107
    style Exec fill:#d4edda,stroke:#28a745
```

**`ExecutionContext` / `RecordOperations`** — the only API surface scripts can call. It is a **synchronous facade** over the reactive `MetadataEngine`/`DynamicPersistenceService`/`QueryEngine`; it `.block()`s internally, which is legal *only* because every call into it is already running on `Schedulers.boundedElastic()` (never the event loop). This is the single, explicitly justified exception to NNG-004 — confined to `backend/kotlin-scripting`, enforced by a dedicated ArchUnit rule (`KotlinScriptingBlockingIsolationRule`) scoped to that module only. Every record operation a script performs still passes through the same FLS/RLS/tenant filtering as the REST API path.

**Why this doesn't violate the reactive non-negotiables:** see ADR-005 §"Why Kotlin Scripting fits the non-negotiable reactive stack". In short — compilation and execution are inherently blocking JVM operations; the engine isolates that blocking behind the elastic scheduler and a hard timeout, exactly the pattern NNG-004/ADR-003 already prescribe for "CPU-bound or legacy-blocking code."

**Sandbox levels** (detail in ADR-005):
1. Compile-time import allowlist (`kotlin.*`, `dev.osc.scripting.api.*`, `java.time.*`, collections — no `java.io.*`, `java.net.*`, `java.lang.reflect.*`).
2. Per-tenant restricted `URLClassLoader`, no access to the host application's full classpath.
3. Runtime guard: wall-clock timeout, CPU sampling, heap allocation check, recursion depth limit.

---

## 10.2. Extended Metadata Model (ERP)

Full design and rationale in **ADR-006**. Additive on top of the Fase 0 schema (§14) — existing objects without record types or relationships are unaffected.

```mermaid
erDiagram
    md_object ||--o{ md_relationship : "child or parent of"
    md_object ||--o{ md_record_type : "has"
    md_record_type ||--o{ md_layout_assignment : "resolves to"
    md_layout ||--o{ md_layout_assignment : "assigned via"
    permission_set ||--o{ md_layout_assignment : "scoped by"
    record }o--o| md_record_type : "record_type_id"

    md_relationship {
        uuid id PK
        text relationship_type "LOOKUP | MASTER_DETAIL | MANY_TO_MANY"
        uuid child_object_id FK
        uuid parent_object_id FK
        uuid field_id FK
        uuid junction_object_id FK
        text on_delete
    }
...
**Field type extensions** (`md_field.field_type`, no new table — `config` JSONB carries the shape):

| Field type | Computed when | Storage |
|---|---|---|
| `FORMULA` | Read time, by the Query Engine, same-object field references only (v1) | Not persisted — `storage_kind` is irrelevant |
| `ROLLUP` | Async, triggered by child-record writes via the outbox/AFTER pipeline (§10 step 8) | Persisted to parent's `data` JSONB / promoted column, eventually consistent |

**Layout resolution order** when rendering a record (most specific wins): `(record_type_id, permission_set_id)` → `(record_type_id, NULL)` → `(NULL, permission_set_id)` → `(NULL, NULL)` object default.

**Many-to-Many** is not a new storage primitive — it is an auto-created junction `md_object` (flagged `config.is_junction = true`, hidden from the standard object picker) with two `MASTER_DETAIL` relationships, consistent with ADR-002's "everything lives in `record`" principle.
...
sequenceDiagram
    ...
    rect rgb(225, 217, 245)
        Note over User,Meta: Use case 3 — NL → Kotlin script (ADR-005)
        User->>AIAPI: "When an Opportunity is won, create a follow-up task"
        AIAPI->>SpringAI: prompt + ExecutionContext API docs + object metadata
        SpringAI-->>AIAPI: Kotlin script source
        AIAPI->>AIAPI: compile-check only (kotlin-scripting, boundedElastic) — no execution
        AIAPI->>AIAPI: static analysis (import allowlist, forbidden APIs, timeout-risk)
        AIAPI-->>User: proposal (source + diagnostics) in Script Editor — md_script.is_active=false
        User->>AIAPI: review, edit, then explicit "Activate"
        AIAPI->>Meta: md_script.is_active = true
    end
```
...
erDiagram
    ...
    md_object ||--o{ md_record_type : "has (ADR-006)"
    md_object ||--o{ md_relationship : "child/parent of (ADR-006)"
    md_object ||--o{ md_script : "has (ADR-005)"
    md_record_type ||--o{ md_layout_assignment : "resolves to (ADR-006)"
    md_layout ||--o{ md_layout_assignment : "assigned via (ADR-006)"
    record }o--o| md_record_type : "record_type_id (ADR-006)"
    md_script ||--o{ script_execution_log : "audited by (ADR-005)"
...
gantt
    ...
    Record Types + Layout assignment   :p4c, after p4b, 2w
    Formula + Rollup fields (ADR-006)  :p4d, after p4c, 2w
...
| Writing a Trigger/Batch/Scheduled job | See §10.1 + ADR-005 — Kotlin Scripting, never the DSL |
| Relationships, Record Types, Page Layouts, Formula/Rollup | See §10.2 + ADR-006 |
| Where `.block()` is legal | **Only** inside `backend/kotlin-scripting`, on `Schedulers.boundedElastic()` — see ADR-005. Nowhere else. |
